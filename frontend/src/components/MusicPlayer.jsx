...
const MusicPlayer = () => {
    ...
    const fetchMusic = async () => {
        try {
            const response = await fetch('http://15.207.249.179:5000/api/music');
            if (!response.ok) {
    ...
    const handleUpload = async (e) => {
        ...
        try {
            const response = await fetch('http://15.207.249.179:5000/api/upload', {
                method: 'POST',
                body: formData,
            });
    ...
    const handleDelete = async (key) => {
        ...
        try {
            const response = await fetch(`http://15.207.249.179:5000/api/music/${key}`, {
                method: 'DELETE',
            });
